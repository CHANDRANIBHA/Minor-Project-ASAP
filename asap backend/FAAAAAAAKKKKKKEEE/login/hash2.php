    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
        ...
        foreach ($marks_data as $topic_id => $evaluations) {
            foreach ($evaluations as $evaluation_id => $mark) {
                ...
                // Check if a mark already exists for this student, topic, evaluation
                $insert_query = "
                INSERT INTO mark_tbl (std_id, subject_id, topic_id, evaluation_id, mark, lr)
                VALUES (?, ?, ?, ?, ?, ?)
                ON DUPLICATE KEY UPDATE mark = VALUES(mark), lr = VALUES(lr)
            ";
...
$stmt->bind_param("iiiddd", $std_id, $subject_id, $topic_id, $evaluation_id, $quantitative, $lr);
$stmt->execute();
