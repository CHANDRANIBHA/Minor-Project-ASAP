...
// Insert into total_tbl for Quantitative
$total_insert_query = "INSERT INTO total_tbl (user_id, topic_id, total, remark, sem) 
                       VALUES (?, ?, ?, ?, ?)";

$stmt_total = $conn->prepare($total_insert_query);

if ($stmt_total === false) {
    die("Error preparing statement for total_tbl: " . $conn->error);
}

// Bind parameters and execute for Quantitative
$stmt_total->bind_param(
    "sidsi",
    $std_id,                // user_id
    $quantitative_topic_id, // topic_id (Quantitative)
    $total_quant,           // total marks for Quantitative
    $remark_quant,          // remark for Quantitative
    $semester               // semester
);

if (!$stmt_total->execute()) {
    echo "Error inserting Quantitative total: " . $stmt_total->error;
}

// Bind parameters and execute for LR
$stmt_total->bind_param(
    "sidsi",
    $std_id,            // user_id
    $lr_topic_id,       // topic_id (LR)
    $total_lr,          // total marks for LR
    $remark_lr,         // remark for LR
    $semester           // semester
);

if (!$stmt_total->execute()) {
    echo "Error inserting LR total: " . $stmt_total->error;
}
