...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (count($errors) === 0) {
        ...
        if ($result->num_rows > 0) {
            ...
            if ($stmt->execute()) {
                // If the role is student, insert into students_tbl with additional student details
                if ($role === "student") {
                    // Insert student details
                    $stmt_student = $conn->prepare("INSERT INTO students_tbl (user_id, username, email, password) VALUES (?, ?, ?, ?)");
                    $stmt_student->bind_param("ssss", $user_id, $username, $email, $password);

                    if ($stmt_student->execute()) {
                        // Redirect to the details page after successful insertion
                        echo "<script>alert('Signup successful! Redirecting to details page.'); window.location.href = 'ifstud.php?user_id=$user_id';</script>";
                    } else {
                        echo "Error adding student details: " . $stmt_student->error;
                        echo "SQL Error: " . mysqli_error($conn); // Show detailed error
                    }
                    $stmt_student->close();
                } else {
