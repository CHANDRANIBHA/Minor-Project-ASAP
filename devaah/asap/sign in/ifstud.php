...
// Database connection
include "C:/XAMPP/htdocs/asap/db.php"; // Update the path if necessary
...
$class_options = [];
$query = "SELECT class_id, class_name, sem FROM class_tbl";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (count($errors) === 0) {
        // Fetch the class_name (batch) and semester for the selected class_id
        $stmt = $conn->prepare("SELECT class_name, sem FROM class_tbl WHERE class_id = ?");
        $stmt->bind_param("i", $class_id);
        $stmt->execute();
        $stmt->bind_result($batch, $semester);  // Here, class_name is used as batch
        $stmt->fetch();
        $stmt->close();

        // If no class found for the given class_id
        if (empty($batch)) {
            $errors['class_id'] = "Invalid class selected.";
        }

        // Proceed if no errors
        if (count($errors) === 0) {
            // Insert or update details in students_tbl where user_id matches
            $stmt = $conn->prepare("INSERT INTO students_tbl (class_id, sem, year_of_joining, batch, user_id) 
                                    VALUES (?, ?, ?, ?, ?)
            ...
            $stmt->bind_param("iiiss", $class_id, $semester, $year_of_joining, $batch, $user_id);
